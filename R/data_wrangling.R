...
#'
#' Calculates the average spectrum from a tidy map data frame
#'
#' @importFrom magrittr %>%
#' @param df_map A tidy dataframe generated by \code{gRdata_map_read()}
...
gRdata_map_avg <- function(df_map, normalise = FALSE) {
  df_map_avg <- df_map %>%
    dplyr::select(-x, -y) %>%
    dplyr::mutate(id = 1:n()) %>%
